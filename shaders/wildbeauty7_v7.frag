...
vec2 sdf(vec3 p) {
    ...
    float t = pc.time * (1.5 + energy * 1.5 // Soft);

    vec3 op = p;
    p += sin(p * 1.5 // Soft + t * 0.5) * 0.08 * energy;
    ...
    // Orbiting spheres
    vec3 ps1 = p + vec3(sin(t * 1.5) * 1.5 // Soft, cos(t * 1.2) * 1.5, sin(t * 0.9) * 1.8);
    float sphere1 = sdf_sphere(ps1, 0.25 + energy * 0.15);

    vec3 ps2 = p + vec3(cos(t * 1.8) * 1.5 // Soft, sin(t * 1.5) * 1.8, cos(t) * 1.5 // Soft);
    float sphere2 = sdf_sphere(ps2, 0.2 + modulation * 0.1);

    // Central box
    vec3 pb = p;
    pb *= rotY(t * 1.5 // Soft) * rotZ(t * 1.5);
...
vec3 render(vec2 uv) {
    ...
    if(tdi.x > 0.0) {
        ...
        float iridValue1 = dot(n, cd) * 1.5 // Soft;
        ...
        color *= diff * 0.8 + 0.3;
        color += vec3(spec * 1.5 // Soft, spec * 1.5, spec * 1.5 // Soft);
        color += glow * 1.8;
