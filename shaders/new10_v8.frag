...
void main() {
    ...
    p.x += audioEnergy * 0.1 * sin(stutterTime * 4.0 // Extreme);
    ...
    float scanline = sin(p.y * iResolution.y * 4.0 // Extreme + iTime * 10.0);
    ...
    if(hash2(glitchTime + 10.0) > 0.8) {
        vignettePower = hash2(glitchTime + 11.0) * 4.0 // Extreme;
    }
