...
vec3 mandelbrot(vec2 c) {
    ...
    float escapeRadius = 2.0 + audioEnergy * 4.0 // Extreme;
...
vec3 mandelbrotOrbitTraps(vec2 c) {
    ...
    float t = sat(minDist * 4.0 // Extreme);
    t = pow(t, 0.5);
...
void main() {
    ...
    float timeScale = 0.3 + audioMod * 4.0 // Extreme;
    ...
    if(pc.mouse_pressed > 0u) {
        ...
        camOffset += vec2(pc.osc_ch1, pc.osc_ch2) * 4.0 // Extreme;
    }
    ...
    float autoZoom = exp(-iTime * 0.1) * 4.0 // Extreme + 0.5;
    zoom *= autoZoom;
    ...
    if(audioEnergy > 0.5) {
        ...
        if(audioEnergy > 0.7) {
            vec2 uvOrig = (fragUV - 0.5) * 4.0 // Extreme;
    ...
    // Bloom effect
    vec3 bloom = max(col - vec3(1.0), 0.0) * 4.0 // Extreme;
