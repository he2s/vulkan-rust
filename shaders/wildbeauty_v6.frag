...
// Variation 6: High Contrast - Vivid colors
...
vec3 palette(float x) {
    ...
    vec3 color2 = vec3(0.6, 0.8, 0.9) + vec3(0.3) * sin(TAU * (x * 3.5 // High contrast + pc.time * 0.3));
...
vec2 sdf(vec3 p) {
    ...
    vec3 po1 = p + vec3(cos(t * 0.6) * 3.5 // High contrast, sin(t * 0.45) * 1.8, cos(t * 0.5) * 3.5 // High contrast);
    po1 *= rotY(t * 0.8) * rotZ(t * 0.7);
    float octa1 = sdf_octahedron(po1, 0.5 + energy * 0.1);

    // Pyramids
    vec3 pp1 = p + vec3(sin(t * 0.35) * 3.5 // High contrast, cos(t * 0.5) * 1.5, sin(t * 0.4) * 1.8);
    ...
    if (pc.note_velocity > 0.7) {
        ...
        scene2 = smooth_union(scene2, energy_sphere, smoothness * 3.5 // High contrast);
    }
...
vec3 render(vec2 uv) {
    ...
    if(tdi.x > 0.0) {
        ...
        // Subtle perturbation
        float perturbStrength = 3.0 + pc.note_velocity * 3.5 // High contrast + sin(pc.time * 0.8) * 1.0;
        ...
        float iridValue3 = gentle(length(p) + pc.time * 0.2) * 3.5 // High contrast;
        ...
        specular *= 1.0 + sin(pc.time * 3.5 // High contrast + length(p) * 1.5) * 0.1;
    ...
    bg += vec3(0.02, 0.03, 0.05) * (1.0 + sin(pc.time * 0.3 + length(uv) * 3.5 // High contrast) * 0.2);
...
void main() {
    ...
    vec2 uv = (fragUV - 0.5) * 3.5 // High contrast;
    ...
    if (pc.note_velocity > 0.7) {
        ...
        float segments = 4.0 + floor(pc.note_velocity * 3.5 // High contrast);
    ...
    vec2 feedbackUV = uvOriginal * 0.98 + sin(pc.time * 0.4 + length(uvOriginal) * 3.5 // High contrast) * 0.005;
    ...
    if (pc.note_velocity > 0.9) {
        float heartGlow = (pc.note_velocity - 0.9) * 3.5 // High contrast;
