...
void main() {
    ...
    p.x += audioEnergy * 0.1 * sin(stutterTime * 3.5 // High contrast);
    ...
    float scanline = sin(p.y * iResolution.y * 3.5 // High contrast + iTime * 10.0);
    ...
    if(hash2(glitchTime + 10.0) > 0.8) {
        vignettePower = hash2(glitchTime + 11.0) * 3.5 // High contrast;
    }
