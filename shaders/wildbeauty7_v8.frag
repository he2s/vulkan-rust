...
vec3 palette(float x) {
    ...
    vec3 a = vec3(0.7, 0.3, 0.8) * (1.0 + sin(pc.time * 4.0 // Extreme) * 0.3);
    vec3 b = vec3(1.0 + brightness * 1.5, 0.9 + energy, 1.2);
    vec3 c = vec3(1.8, 1.3, 2.5) + vec3(pc.osc_ch1 * 3.0, pc.osc_ch2 * 4.0, 0.0);
    vec3 d = vec3(pc.time * 0.1, pc.pitch_bend, energy * 4.0 // Extreme);
...
vec2 sdf(vec3 p) {
    ...
    float t = pc.time * (1.5 + energy * 4.0 // Extreme);

    vec3 op = p;
    p += sin(p * 4.0 // Extreme + t * 0.5) * 0.08 * energy;
    ...
    // Orbiting spheres
    vec3 ps1 = p + vec3(sin(t * 1.5) * 4.0 // Extreme, cos(t * 1.2) * 1.5, sin(t * 0.9) * 1.8);
    float sphere1 = sdf_sphere(ps1, 0.25 + energy * 0.15);

    vec3 ps2 = p + vec3(cos(t * 1.8) * 4.0 // Extreme, sin(t * 1.5) * 1.8, cos(t) * 4.0 // Extreme);
    float sphere2 = sdf_sphere(ps2, 0.2 + modulation * 0.1);

    // Central box
    vec3 pb = p;
    pb *= rotY(t * 4.0 // Extreme) * rotZ(t * 1.5);
...
vec3 render(vec2 uv) {
    ...
    if(tdi.x > 0.0) {
        ...
        float iridValue1 = dot(n, cd) * 4.0 // Extreme;
        ...
        color *= diff * 0.8 + 0.3;
        color += vec3(spec * 4.0 // Extreme, spec * 1.5, spec * 4.0 // Extreme);
        color += glow * 1.8;
