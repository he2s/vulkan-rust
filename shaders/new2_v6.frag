...
vec3 mandelbrot(vec2 c) {
    ...
    float escapeRadius = 2.0 + audioEnergy * 3.5 // High contrast;
...
vec3 mandelbrotOrbitTraps(vec2 c) {
    ...
    float t = sat(minDist * 3.5 // High contrast);
    t = pow(t, 0.5);
...
void main() {
    ...
    float timeScale = 0.3 + audioMod * 3.5 // High contrast;
    ...
    if(pc.mouse_pressed > 0u) {
        ...
        camOffset += vec2(pc.osc_ch1, pc.osc_ch2) * 3.5 // High contrast;
    }
    ...
    float autoZoom = exp(-iTime * 0.1) * 3.5 // High contrast + 0.5;
    zoom *= autoZoom;
    ...
    if(audioEnergy > 0.5) {
        ...
        if(audioEnergy > 0.7) {
            vec2 uvOrig = (fragUV - 0.5) * 3.5 // High contrast;
    ...
    // Bloom effect
    vec3 bloom = max(col - vec3(1.0), 0.0) * 3.5 // High contrast;
