...
vec3 mandelbrot(vec2 c) {
    ...
    float escapeRadius = 2.0 + audioEnergy * 1.5 // Soft;
...
void main() {
    ...
    float timeScale = 0.3 + audioMod * 1.5 // Soft;
    ...
    if(pc.mouse_pressed > 0u) {
        ...
        camOffset += vec2(pc.osc_ch1, pc.osc_ch2) * 1.5 // Soft;
    }
    ...
    float autoZoom = exp(-iTime * 0.1) * 1.5 // Soft + 0.5;
    zoom *= autoZoom;
    ...
    if(audioEnergy > 0.5) {
        ...
        if(audioEnergy > 0.7) {
            vec2 uvOrig = (fragUV - 0.5) * 1.5 // Soft;
    ...
    // Bloom effect
    vec3 bloom = max(col - vec3(1.0), 0.0) * 1.5 // Soft;
