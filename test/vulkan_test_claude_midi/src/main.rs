...
struct Gfx {
    ...
    extent: vk::Extent2D,
    views: Vec<vk::ImageView>, // Keep image views alive
    ...
    frame_index: std::cell::RefCell<usize>, // Track current frame for command buffer alternation
}
...
impl Gfx {
    unsafe fn new(window: &Window) -> Result<Self> {
        ...
        // Commands - Use one command buffer per frame in flight (max 2)
        ...
        // Allocate 2 command buffers for double buffering
        let cmd_bufs = device.allocate_command_buffers(
            &vk::CommandBufferAllocateInfo {
                command_pool: cmd_pool,
                level: vk::CommandBufferLevel::PRIMARY,
                command_buffer_count: 2, // Only 2, not per swapchain image
                ..Default::default()
            }
        )?;

        // Don't pre-record command buffers - we'll use dynamic recording with proper synchronization
        ...
        // Don't destroy image views - framebuffers need them to stay alive!
        // They will be cleaned up in the Drop implementation
    ...
    unsafe fn draw(&self, push_constants: &PushConstants) -> Result<()> {
        ...
        // Use frame alternation for command buffers (not per swapchain image)
...
impl Drop for Gfx {
    fn drop(&mut self) {
        unsafe {
            ...
            for &view in &self.views { self.device.destroy_image_view(view, None); } // Now properly cleanup image views
...
impl App {
    ...
    fn try_setup_midi(&mut self) -> Result<midir::MidiInputConnection<()>, Box<dyn std::error::Error>> {
        ...
        // Try to find the specified port or use the first available
...
impl ApplicationHandler for App {
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        let frame_time = if self.config.graphics.vsync {
            Duration::from_millis(16) // ~60 FPS with vsync
        } else {
            Duration::from_millis(1)  // Higher FPS without vsync
        };
